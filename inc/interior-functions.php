...
function interior_opt( $id = null, $default = '' ) {
...
// Blog Date Permalink
function interior_blog_date_permalink() {
...
// Blog Excerpt Length
if ( ! function_exists( 'interior_excerpt_length' ) ) {
	function interior_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'interior_posted_comments' ) ) :
    function interior_posted_comments() {
        
        $comments_num = get_comments_number();
        if( comments_open() ) {
            if( $comments_num == 0 ) {
                $comments = esc_html__('0 Comment','interior');
            } elseif ( $comments_num > 1 ) {
                $comments= $comments_num . esc_html__(' Comments','interior');
            } else {
                $comments = esc_html__( '1 Comment','interior' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a> <span class="lnr lnr-bubble"></span>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'interior' );
        }
...
function interior_iframe_match() {
    $audio_content = interior_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function interior_embedded_media( $type = array() ) {
...
// WP post link pages
function interior_link_pages() {
    
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'interior' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'interior' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// theme logo
function interior_theme_logo( $class = '' ) {
...
function interior_pull_right( $id = '', $condation ) {
...
//  wysiwyg content output use in interior companion
function interior_get_textareahtml_output( $content ) {
...
function interior_get_slider_shortcode_options( $field ) {
...
// html Style tag for background image use in interior companion plugin
function interior_inline_bg_img( $bgUrl ) {
    $bg = '';
...
function interior_sidebar_opt() {

    $sidebarOpt = interior_opt( 'interior-blog-sidebar-settings' );
    $sidebar = '1';
