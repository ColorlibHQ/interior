...
final class Interior {
	...
	private function setup() {
		...
		$enqueu = new Interior_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->interior_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'interior_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'interior', INTERIOR_DIR_PATH . '/languages' );
		...
		add_image_size( 'interior_blogsect_post_thumb', 295, 211, true );
		
		// Custom thumbnails size for widget post
		add_image_size( 'interior_widget_post_thumb', 75, 75, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'interior-np-thumb', 59, 59, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'interior' ),
            'social-menu'   => esc_html__( 'Social Menu', 'interior' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = INTERIOR_DIR_CSS_URI;
		$jsPath  = INTERIOR_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					...
					'dependency' 	=> array(),
					'version' 		=> $this->interior_version,
				),
				array(
					'handler'		=> 'interior-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->interior_version,
				),
				array(
					'handler'		=> 'interior-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'interior' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate interior theme customizer
		$interior_theme_customizer = new interior_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'interior_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'interior_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'interior-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'interior_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'interior-elementor-default-style',  INTERIOR_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->interior_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'interior_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'interior-elementor-notice', INTERIOR_DIR_JS_URI.'interior-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'interior-elementor-notice',
				'interiorElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'interior-elementor-notice-nonce' ),
				)
			);
		}
